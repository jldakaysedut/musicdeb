...
export default function Dashboard() {
  ...
  const fetchTracks = async () => {
    // Kinukuha na lang natin muna yung mga 'approved' na tracks kung public feed ito, pero dahil profile mo to, kunin natin lahat muna
  ...
  // --- CRUD OPERATIONS (UPDATED WITH USER_ID) ---
  const handleUpload = async (e) => {
    ...
    if (!uploadError) {
      ...
      // I-link ang upload sa user.id at i-set as pending by default
      await supabase.from('tracks').insert([{ 
        ...
        is_favorite: false,
        user_id: user.id,
        status: 'pending' // Ready for Phase 3 Admin Approval
      }])
  ...
  return (
    <div className="min-h-screen bg-[#090909] text-white font-sans pb-40 overflow-x-hidden selection:bg-green-500 selection:text-black">
      ...
      <div className="max-w-md md:max-w-4xl mx-auto p-6">
        ...
        <div className="flex justify-between items-center mb-10 pt-4 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <div className="flex items-center gap-3">
            {/* UPDATED: PROFILE AND LOGOUT BUTTONS */}
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
            <button onClick={() => setFilter('All')} 
              className={`px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border ${filter === 'All' ? 'bg-green-500 text-black border-green-400 shadow-[0_5px_15px_rgba(34,197,94,0.3)]' : 'bg-[#121212] text-gray-300 border-[#222] hover:border-gray-500'}`}>
              All Tracks
            </button>
            <button onClick={() => setFilter('Favorites')} 
              className={`px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border flex items-center gap-2 ${filter === 'Favorites' ? 'bg-green-500 text-black border-green-400 shadow-[0_5px_15px_rgba(34,197,94,0.3)]' : 'bg-[#121212] text-gray-300 border-[#222] hover:border-gray-500'}`}>
              <Heart size={16} className={filter === 'Favorites' ? 'fill-black' : ''} /> Favorites
            </button>
            <button onClick={() => setFilter('Radio')} 
              className={`px-6 py-3 rounded-2xl font-bold text-sm transition-all whitespace-nowrap border flex items-center gap-2 ${filter === 'Radio' ? 'bg-green-500 text-black border-green-400 shadow-[0_5px_15px_rgba(34,197,94,0.3)]' : 'bg-[#121212] text-gray-300 border-[#222] hover:border-gray-500'}`}>
        ...
        <div className="flex flex-col gap-3 pb-10 animate-in fade-in duration-1000">
          <div className="flex justify-between items-center mb-2 px-2">
            <span className="text-gray-400 text-sm font-medium">{filter === 'Radio' ? 'Trending Stations' : 'Track List'}</span>
                ...
                <div key={item.id} onClick={() => playTrack(originalIndex, currentListType)}
                  ...
                  <div className="flex items-center gap-4 overflow-hidden w-full">
                    ...
                    <div className="overflow-hidden pr-2">
                      ...
                      <p className={`text-xs truncate mt-0.5 font-medium ${item.is_radio ? 'text-green-500/70' : 'text-gray-500'}`}>
                        {item.artist} {item.status === 'pending' && <span className="ml-2 px-2 py-0.5 bg-yellow-500/10 text-yellow-500 text-[10px] rounded-full">Pending</span>}
                      </p>
                    </div>
                  </div>
      ...
      <button onClick={() => setShowUpload(true)}
              className="fixed bottom-[140px] md:bottom-32 right-6 w-14 h-14 bg-green-500 rounded-full flex items-center justify-center shadow-[0_10px_30px_rgba(34,197,94,0.4)] hover:scale-105 hover:bg-green-400 transition-all z-40 text-black">
      ...
      {showUpload && (
        <div className="fixed inset-0 bg-[#090909]/90 backdrop-blur-md z-50 flex items-center justify-center p-5 animate-in fade-in duration-300">
          <div className="bg-[#121212] border border-[#222] w-full max-w-sm p-8 rounded-[2rem] shadow-2xl relative">
            <button onClick={() => setShowUpload(false)} className="absolute top-6 right-6 p-2 text-gray-500 hover:text-white transition-colors bg-[#1a1a1a] rounded-full">
              <X size={20} />
            </button>
            ...
            <form onSubmit={handleUpload} className="flex flex-col gap-4">
              <input type="text" placeholder="Song Title" required value={title} onChange={(e) => setTitle(e.target.value)}
                className="p-4 bg-[#1a1a1a] rounded-2xl focus:outline-none border border-[#222] focus:border-green-500 text-white font-medium transition-colors" />
              ...
              <button type="submit" disabled={uploading}
                className="mt-6 py-4 bg-green-500 text-black font-extrabold text-lg rounded-2xl hover:bg-green-400 transition-all disabled:opacity-50 shadow-[0_10px_20px_rgba(34,197,94,0.2)]">
      ...
      {currentTrack && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-3rem)] max-w-2xl bg-[#121212]/95 backdrop-blur-3xl border border-[#222] rounded-[2rem] p-4 shadow-[0_20px_50px_rgba(0,0,0,0.8)] z-50 animate-in slide-in-from-bottom-10">
          <div className="w-full h-1.5 bg-[#222] rounded-full mb-4 overflow-hidden">
            <div className={`h-full transition-all duration-300 relative ${currentTrack.is_radio ? 'bg-green-500/50 w-full animate-pulse' : 'bg-green-500'}`} style={{ width: currentTrack.is_radio ? '100%' : `${progress}%` }}>
            </div>
          </div>
