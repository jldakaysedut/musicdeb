  ...
  const handleUpload = async (e) => {
    ...
    if (!uploadError) {
      ...
      await supabase.from('tracks').insert([{ 
        title, artist, file_url: urlData.publicUrl, is_favorite: false,
    ...
    <div className="min-h-screen bg-[#050505] text-white font-sans pb-44 overflow-x-hidden selection:bg-orange-500">
      ...
      <header className="max-w-5xl mx-auto p-6 flex justify-between items-center animate-in fade-in slide-in-from-top-4 duration-700">
        ...
        <div className="hidden md:flex items-center gap-3">
           <Link to="/leaderboard" className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center border border-white/5 hover:border-orange-500/50 transition-colors">
            <Trophy size={18} />
          </Link>
          <Link to="/chat" className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center border border-white/5 hover:border-orange-500/50 transition-colors">
            <MessageSquare size={18} />
          </Link>
          <Link to="/profile" className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center border border-white/5 hover:border-orange-500/50 transition-colors">
            <User size={18} />
          </Link>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-6">
        ...
        {/* 2. CATEGORY PILLS */}
        <div className="flex gap-3 overflow-x-auto py-4 scrollbar-hide no-scrollbar">
        ...
        {/* 3. TRENDING / EXPLORE (Mobile Carousel) */}
        {filter === 'All' && tracks.length > 0 && (
          <section className="mt-8">
            <h2 className="text-sm font-black uppercase tracking-[0.2em] text-gray-500 mb-4 px-1">Trending Vaults</h2>
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x">
              {tracks.slice(0, 4).map((track, i) => (
                <div key={track.id} onClick={() => playTrack(i, 'tracks')}
                  className="min-w-[160px] h-[160px] bg-[#0A0A0A] border border-white/5 rounded-[2rem] p-5 flex flex-col justify-end relative overflow-hidden group cursor-pointer snap-center">
                  ...
                  <Music size={20} className="absolute top-5 right-5 text-orange-500/30" />
                  <h3 className="font-black text-sm truncate leading-tight">{track.title}</h3>
                  <p className="text-[10px] font-bold text-gray-500 uppercase mt-1 truncate">@{track.profiles?.username}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="mt-10">
          <div className="flex justify-between items-center mb-6 px-1">
            ...
            <span className="text-[10px] font-bold text-gray-600">{displayedItems.length} ITEMS</span>
          </div>
          ...
          <div className="space-y-3">
            {displayedItems.length === 0 ? (
              <div className="text-center py-20 bg-white/[0.02] rounded-[2.5rem] border border-white/5 border-dashed">
                ...
                <p className="text-gray-500 font-bold text-sm">Your library is quiet... let's add some music!</p>
              </div>
            ) : (
              displayedItems.map((item, index) => {
                ...
                return (
                  <div key={item.id} onClick={() => playTrack(index, filter === 'Radio' ? 'radios' : 'tracks')}
                    ...
                    ${isPlayingThis ? 'bg-orange-500/10 border-orange-500/30 shadow-lg' : 'bg-white/[0.03] border-transparent hover:border-white/10'}`}>
                    <div className="flex items-center gap-4 overflow-hidden">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 border border-white/5 ${isPlayingThis ? 'bg-orange-500 text-black' : 'bg-[#111]'}`}>
                        {isPlayingThis && isPlaying ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" className={isPlayingThis ? "" : "text-gray-600"} />}
                      </div>
                      <div className="truncate">
                        <h4 className={`text-sm font-black truncate ${isPlayingThis ? 'text-orange-500' : 'text-white'}`}>{item.title}</h4>
                        <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-0.5">{item.artist}</p>
                      </div>
                    </div>
                    {item.status === 'pending' && <span className="text-[8px] font-black bg-orange-500/20 text-orange-500 px-2 py-1 rounded-md uppercase">Pending Review</span>}
                  </div>
                )
              })
            )}
      ...
      {/* 6. MODAL: UPLOAD SOUND */}
      {showUpload && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-xl z-50 flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-[#0A0A0A] border border-white/10 w-full max-w-sm p-8 rounded-[2.5rem] relative shadow-2xl">
            <button onClick={() => setShowUpload(false)} className="absolute top-6 right-6 text-gray-500 hover:text-white"><X size={24} /></button>
            <h2 className="text-2xl font-black mb-2 tracking-tighter">SHARE YOUR SOUND.</h2>
            <p className="text-xs font-bold text-gray-500 mb-6 uppercase tracking-widest">Contribute to the vault.</p>
            <form onSubmit={handleUpload} className="space-y-4">
              <input type="text" placeholder="Song Title" required value={title} onChange={(e) => setTitle(e.target.value)} className="w-full p-4 bg-white/5 rounded-2xl border border-white/5 focus:border-orange-500 outline-none text-sm font-bold" />
              <input type="text" placeholder="Artist Name" required value={artist} onChange={(e) => setArtist(e.target.value)} className="w-full p-4 bg-white/5 rounded-2xl border border-white/5 focus:border-orange-500 outline-none text-sm font-bold" />
              <div className="relative border-2 border-dashed border-white/5 rounded-2xl p-6 text-center bg-white/[0.02] hover:bg-white/5 transition-colors">
                ...
                <Music size={24} className="mx-auto mb-2 text-gray-700" />
                <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{file ? file.name : "Select MP3 File"}</p>
              </div>
              <button type="submit" disabled={uploading} className="w-full py-4 bg-orange-500 text-black font-black rounded-2xl hover:bg-orange-400 disabled:opacity-50 transition-all shadow-lg">
      ...
      {currentTrack && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 w-[92%] max-w-2xl bg-black/80 backdrop-blur-3xl border border-white/10 rounded-[2rem] p-4 shadow-2xl z-40 animate-in slide-in-from-bottom-10">
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              <div className="truncate">
                <h4 className="text-xs font-black truncate">{currentTrack.title}</h4>
                <p className="text-[9px] font-bold text-gray-500 uppercase tracking-widest truncate">{currentTrack.artist}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <button onClick={handlePrev} className="text-gray-400 hover:text-white"><SkipBack size={20} fill="currentColor" /></button>
              <button onClick={togglePlayPause} className="w-10 h-10 bg-white text-black rounded-full flex items-center justify-center shadow-lg active:scale-90">
                {isPlaying ? <Pause size={18} fill="currentColor" /> : <Play size={18} fill="currentColor" className="ml-0.5" />}
              </button>
              <button onClick={handleNext} className="text-gray-400 hover:text-white"><SkipForward size={20} fill="currentColor" /></button>
            </div>
            <audio ref={audioRef} src={currentTrack.file_url} onTimeUpdate={onTimeUpdate} className="hidden" />
          </div>
        </div>
      )}
